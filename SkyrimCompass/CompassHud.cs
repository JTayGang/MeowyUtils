...
public sealed class CompassHud : IDisposable
{
    ...
    // cached target label (name + level)
    ...
    private readonly Dictionary<int, (string Name, string Desc)> _iconToStatusText = new(); // for stacked vanilla statuses
        ...
        Configuration cfg, IPluginLog log, IFontHandle font, IDalamudPluginInterface pi)
    {
        ...
        if (statusSheet != null)
            ...
            {
                int icon = (int)row.Icon;
                if (icon>0 && !_iconToStatusText.ContainsKey(icon))
                {
                    string name = row.Name.ToString();
                    if (!string.IsNullOrEmpty(name))
                        _iconToStatusText[icon] = (name, row.Description.ToString());
                }
            }
    }
    ...
    public unsafe void Draw()
    {
        ...
        float bw = _cfg.CompassWidth, bh = _cfg.CompassHeight;
    ...
    private float UpdateCtxFade(float now)
    {
        ...
        return Lerp(open ? 1f : CtxDimAlpha, open ? CtxDimAlpha : 1f, SmoothStep(t));
    }

    private bool IsCtxMenuOpen() => _gg.GetAddonByName("ContextMenu").IsVisible || _gg.GetAddonByName("AddonContextSub").IsVisible;
    ...
    private static float Project(float d, float halfVis, float halfW, float lens)
    {
        ...
        float u = absD / ext;
        return (d >=0f ? 1f : -1f) * halfW * (1f - MathF.Pow(1f - u, lens));
    }
        ...
        IPlayerCharacter player, Vector3 origin, float now, bool inDuty)
    {
        ...
        float halfW = bw*0.5f, halfVis = _cfg.VisibleDegrees*0.5f, lens = _cfg.LensStrength, ext = halfVis * lens;
        uint bg=C(_cfg.BackgroundColor), border=C(_cfg.BorderColor), tick=C(_cfg.TickColor), card=C(_cfg.CardinalColor), inter=C(_cfg.IntercardinalColor);
        ...
        {
            float t=now;
            _lbLayers[0]=(Math.Clamp(lbProg,0f,1f),1.00f,0f,_cfg.LimitBreakGlowColor);
        ...
        float intensity=1f, bool fromLeft=true, float wipe=0f, float fill=0f, bool wipeRev=false)
    {
        ...
        float len=d.Length();
        if (len<1f) return;
        Vector2 dir=d/len, perp=new(-dir.Y, dir.X);
        ...
        float freq = 2f*MathF.PI/waveLen, freq2 = freq*2f;
        for (int i=0; i<samples; i++)
        {
            float along = len * i / (samples-1);
            float u = fromLeft ? along/len : 1f - along/len;
            float env = u*u*(3f-2f*u);
            float wave = MathF.Sin(along*freq + phase)*(1f - harm*u) + MathF.Sin(along*freq2 + phase2)*(harm*u);
        ...
        float real, bool trigger, bool extResync, bool resyncIfExceeds, float now, float dur, out float wipe)
    {
        ...
        if (start >= 0f)
        {
            ...
            { start = -1f; tracked = real; wipe = 0f; return tracked; }
            wipe = elapsed / dur;
            return frozen;
        }
        wipe = 0f;
    ...
    private bool IsInDutyOrPvp() => _cond[ConditionFlag.BoundByDuty] || _cond[ConditionFlag.BoundByDuty56] ||
        _cond[ConditionFlag.BoundByDuty95] || _cond[ConditionFlag.InDeepDungeon] || _cs.IsPvP;
    ...
    private (float x, float w, float totX, float totW, float rowW) SplitTargetRow(float cx, float cw, bool hasTot)
    {
        ...
        return (rowX, mainW, rowX + mainW + gap, totW, rowW);
    }
    ...
    {
        lo=Math.Clamp(lo,0f,1f); hi=Math.Clamp(hi,0f,1f);
        float botX=x+taper, botSpan=w-2f*taper;
        float topA=x+w*lo, topB=x+w*hi, botA=botX+botSpan*lo, botB=botX+botSpan*hi;
    ...
    private (Vector2 tl,Vector2 tr,Vector2 br,Vector2 bl) TrapFill(float x,float y,float w,float h,float taper,float frac,bool fromRight=false)
    {
        ...
        return fromRight ? TrapSlice(x,y,w,h,taper,1f-frac,1f) : TrapSlice(x,y,w,h,taper,0f,frac);
    }
        ...
        uint bg,uint fg,float alpha, float? shieldFrac=null, uint? shieldCol=null, bool fromRight=false)
    {
        ...
        float innerH=h-inset*2f, innerTaper=taper*(innerH/h);
    ...
    private void UpdateCompassDrag()
    {
        ...
        if (ImGui.IsMouseReleased(ImGuiMouseButton.Left)) { _isDraggingCompass=false; _cfg.Save(_pi); }
    }
        ...
        float nameRowW,float now,float alpha,bool inDuty, IGameObject? curTarget)
    {
        ...
        uint border=WithAlpha(C(_cfg.BorderColor), alpha), bg=WithAlpha(C(_cfg.BackgroundColor), alpha), nameCol=WithAlpha(C(_cfg.CardinalColor), alpha);
        ...
        if (isChara)
        {
            ...
            if (isMinion) { raw=1f; _dispTargetHp=1f; _lastRawHp=1f; _flashAlpha=0f; rawShield=0f; _dispShield=0f; }
            float dt=ImGui.GetIO().DeltaTime;
            if (curTarget.GameObjectId != _lastTargetId)
            { _lastTargetId=curTarget.GameObjectId; _dispTargetHp=raw; _lastRawHp=raw; _flashAlpha=0f; _dispShield=rawShield; }
            else
            {
            ...
            if (!isMinion) _flashAlpha = MathF.Max(0f, _flashAlpha - dt/0.4f);
            ...
            {
                float lo=MathF.Min(raw,_dispTargetHp), hi=MathF.Max(raw,_dispTargetHp);
                if (hi>lo)
                {
                    float taper=MathF.Min(h*0.9f,w*0.35f);
                    const float inset=2f;
        ...
        float nameY=y+h+nameGap, tx=nameCx - sz.X*0.5f;
        ...
        if (isChara && _cfg.ShowTargetBarRibbons)
        {
            ...
            float rL=MathF.Min(rowLeft+inset,leftEdge-24f), rR=MathF.Max(rowRight-inset,rightEdge+24f);
            float t=now;
            ...
            { _castTarget=curTarget.GameObjectId; _castTracked=0f; _castFadeStart=-1f; }
            var bt=curTarget as IBattleChara;
            bool isCasting=bt?.IsCasting ?? false;
            float totalCast=bt?.TotalCastTime ?? 0f;
            float castReal=0f;
            if (isCasting && totalCast>0f) castReal = Math.Clamp((bt?.CurrentCastTime ?? 0f) / totalCast,0f,1f);
            float castDisp = UpdateFadeOut(ref _castTracked, ref _castFrozen, ref _castFadeStart,
                castReal, !isCasting && _castTracked>0f, isCasting, false, t, CastFadeDur, out float castWipe);
            if (castDisp>0f)
            {
                uint castCol=WithAlpha(C(_cfg.AggroWarningColor),alpha);
                float ci=PulseIntensity(t);
                DrawFlowingLine(dl,V(leftEdge,textCy),V(Lerp(leftEdge,rL,castDisp),textCy),castCol,t,1f,0f,ci,true,castWipe,0f,true);
                DrawFlowingLine(dl,V(rightEdge,textCy),V(Lerp(rightEdge,rR,castDisp),textCy),castCol,t,1f,0f,ci,true,castWipe,0f,true);
            }
        }
        ...
        float clickLeft=MathF.Min(x,leftX-shHW), clickRight=MathF.Max(x+w,rightX+shHW);
        ...
        IPlayerCharacter player,float now,float alpha,bool inDuty, IGameObject curTarget, IGameObject tot)
    {
        ...
        if (_cfg.ShowTargetOfTargetName)
        {
            ...
            uint nameCol=WithAlpha(C(_cfg.CardinalColor),alpha), shadow=WithAlpha(0xCC000000u,alpha);
            foreach (var (dx,dy) in ShadowOffsets8)
    ...
    private void RenderStatuses(ImDrawListPtr dl, IBattleChara ch, float cx, float y, float alpha)
    {
        ...
        var font=ImGui.GetFont();
        float textGap=-size*0.12f, tipGap=MathF.Max(4f,size*0.15f);
        ...
        for (int i=0; i<n; i++)
        {
            ...
            if (stacks>1)
            {
                ...
                {
                    int candidate=icon + Math.Min(stacks, maxStacks) - 1;
                    if (_iconToStatusText.TryGetValue(candidate, out var data))
                    {
                        if (string.Equals(data.Name, name, StringComparison.OrdinalIgnoreCase))
                        { displayIcon=candidate; displayName=data.Name; displayDesc=data.Desc; }
                    }
                    else displayIcon=candidate;
                }
            }
            ...
            if (durLabel != null)
            {
                ...
                float lx=sx-lsz.X*0.5f, ly=scy+halfH+textGap;
    ...
    private bool IsVerOk(PluginIpcState st, float now)
    {
        ...
        try
        {
            ...
            else { _lociVer.InvokeFunc(); st.VerOk=true; }
        }
    ...
    private void AppendPluginStatuses<T>(nint addr,int max,float now,
        ...
        Func<T,long> expireSel, Func<T,int> stacksSel)
    {
        ...
        {
            cacheTarget=addr; cacheAt=now;
            try { cache=getter.InvokeFunc(addr); } catch { cache=null; }
    ...
    { for (int i=0;i<_statusBuf.Count;i++) if (_statusBuf[i].Guid==g) return true; return false; }
        ...
        System.Guid g, long expireMs, float now)
    {
        ...
        if (!tracker.TryGetValue(g, out var tracked) || tracked.TotalMs != expireMs)
            tracker[g] = (now, expireMs);
        return MathF.Max(0f, expireMs/1000f - (now - tracker[g].FirstSeen));
    }

    private void PruneTrackers(float now) { if (now<_nextPruneAt) return; _nextPruneAt=now+60f; PruneTracker(_moodleTrack,now); PruneTracker(_lociTrack,now); }
    ...
    private static bool TryResolveColor(string val, out ushort key) => ushort.TryParse(val,out key) || _namedColors.TryGetValue(val,out key);
    ...
    private byte[] GetFormattedTooltip(string name,string desc)
    {
        ...
        if (!string.IsNullOrWhiteSpace(desc)) { sb.AddText("\n"); AppendFormat(sb,desc); }
        var bytes=sb.Encode();
    ...
    {
        float maxDist=_cfg.MaxMarkerDistance, maxDistSq=maxDist*maxDist;
        float fateMax=maxDist*_cfg.FateDistanceMultiplier, fateMaxSq=fateMax*fateMax;
        float ext=halfVis*lens;
        ...
        foreach (var cand in _candidates)
        {
            ...
            if (cand.Fate is { } fate)
            {
                ...
                if (!(fate.IconId>0 && TryDrawIcon(dl,(int)fate.IconId,sx,cy,size,alpha,false,1f,true)))
            ...
            var obj=cand.Obj!;
            uint col=cand.Col;
            int iconId=0; float iconSize=0f;
            ...
            bool drew=iconId>0 && TryDrawIcon(dl,iconId,sx,cy,iconSize,alpha,false,1f,true);
                ...
                if (obj.ObjectKind==ObjectKind.Pc)
                {
                    ...
                    if (showRole && obj is ICharacter ch && (ch.StatusFlags & StatusFlags.PartyMember)!=0)
                    {
                        ...
                        if (jobIcon>0)
                        {
                            ...
                            TryDrawIcon(dl,jobIcon,sx,cy,drawSize,alpha,false,1f,true);
                            drewJob=true;
                        }
                    }
                    if (!drewJob)
    ...
    private bool TryDrawIcon(ImDrawListPtr dl,int iconId,float sx,float cy,float size,float alpha,
        bool clipCircle=false,float zoom=1f,bool unclip=true)
    {
        ...
        float hw,hh; Vector2 uvMin,uvMax;
        if (clipCircle)
        {
            hw=hh=size*0.5f;
    ...
    private bool MatchesKw(uint baseId,string[] kw) => HasKw(GetTitle(baseId),kw) || HasKw(GetSingular(baseId),kw);
    ...
    private static void DrawDot(ImDrawListPtr dl,float sx,float cy,float size,uint col,float alpha,bool filled)
    {
        ...
        else dl.AddCircle(V(sx,cy), r, WithAlpha(col,alpha), 0, 2f);
        ...
        uint? ring,uint? shadow,float alpha)
    {
        ...
        if (ring is { } r) dl.AddCircle(V(sx,cy), half+1f, WithAlpha(r,alpha), 0, 3f);
        PopUnclip(dl);
    }
