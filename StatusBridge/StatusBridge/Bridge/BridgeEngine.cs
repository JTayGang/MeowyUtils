...
internal sealed class BridgeEngine : IDisposable
{
    ...
    private void SyncMoodlesToLoci(List<MoodlesStatusInfo> moodles, List<LociStatusInfo> lociExisting)
    {
        ...
        foreach (var guid in _mirroredIntoLoci.Keys.Where(g => !nativeGuids.Contains(g)).ToList())
        {
            if (lociExisting.Any(l => l.GUID == guid))
            {
                ...
                // effect (see LociIpc.TryRemove/MoodlesIpc.TryRemove remarks - the Moodles side
                // in particular is a fire-and-forget void call with zero feedback, and can
                // silently no-op entirely, e.g. while a synced character's data is flagged
                // Ephemeral). Leaving it tracked means we simply try again next tick and only
    ...
    public void ClearAllMirrors()
    {
        ...
        // are actually gone. This used to Clear() both dictionaries unconditionally right after
        // firing the TryRemove calls, with no check that they'd actually taken effect - which is
        // exactly backwards for anything Moodles silently no-ops (see the cleanup-loop comments
        // above). Untracking something that's still really there doesn't just leave a stray
        // status behind: it makes the very next reconcile pass see a "new" entry nothing is
        // tracking, decide it must be native, and mirror it to the other side - i.e. this method
        // could previously turn a stuck ghost into a freshly (re-)created one instead of removing
        // it. Anything still present after this re-check stays tracked, so the normal per-tick
